...
export function KanbanBoard({
  ...
  function onDrop(e: React.DragEvent, colStatus: Task["status"]) {
    ...
    // Compute new position (fractional midpoint)
    ...
    setTasks((prev) =>
      prev.map((t) =>
        t.id === dragId.current
          ? { ...t, status: colStatus, position: newPosition }
          : t,
      ),
    );
  ...
  return (
    <div>
                  ...
                  <div className="mb-2">
                    ...
                    <div className="mt-1.5 flex gap-1.5">
                      <button
                        ...
                        className="rounded-md bg-pin-red px-2.5 py-1 font-mono text-xs text-white"
                      >
                        Add
                      </button>
                      <button
                        onClick={() => { setAdding(false); setNewTitle(""); }}
                        className="rounded-md border border-paper-edge px-2.5 py-1 font-mono text-xs text-ink-soft"
                      >
                        Cancel
                      </button>
                    </div>
              ...
              <div className="space-y-2">
                {colTasks.map((task) => (
                  <div
                    ...
                    className="group cursor-grab rounded-md border border-paper-edge bg-paper p-2.5 shadow-sm active:cursor-grabbing active:opacity-60"
                  >
                    <p className="text-sm text-ink leading-snug">{task.title}</p>
                    <div className="mt-2 flex items-center justify-between gap-2">
                      {task.assigneeId ? (
                        <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-pin-teal text-[0.6rem] font-semibold text-white">
      ...
      <p className="mt-3 font-mono text-xs text-ink-soft">
        Drag cards between columns · use ← → buttons for keyboard access.
      </p>
    </div>
  );
}
